<template>
  <v-card :color="courseColor" dark width="380" height="260" class="ma-4 pa-2">
    ...
    <v-card-actions>
      ...
      <router-link :to="`/teacher/discuss/${courseId}`">
        <v-btn text v-show="manageable">讨论区</v-btn>
      </router-link>
      <router-link :to="`/teacher/createQuestions/${courseId}`">
        <v-btn text v-show="manageable">题库</v-btn>
      </router-link>
    </v-card-actions>
  </v-card>
</template>
