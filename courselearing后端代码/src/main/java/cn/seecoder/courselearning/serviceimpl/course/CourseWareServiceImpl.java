...
public class CourseWareServiceImpl implements CourseWareService {
    ...
    public ResultVO<CourseWareVO> createCourseWare(CourseWareVO courseWareVO) {
        ...
        if(!FileConstant.FileTypeIsSupported(courseWare.getFileType())){
            System.out.println(courseWare);
            return new ResultVO<>(Constant.REQUEST_FAIL,"课件创建失败，因为课件格式不支持");
        }
    ...
    public ResultVO<String> deleteCourseWare(Integer courseWareId) {
        if(courseWareMapper.selectByPrimaryKey(courseWareId) == null)
            ...
            //先删除文件
            String filename=courseWareMapper.selectByPrimaryKey(courseWareId).getFileName();
